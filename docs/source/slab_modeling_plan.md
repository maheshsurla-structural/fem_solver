...
| BE6 | Composite/eccentric T-beam (insertion-point offset) | GUI+wire | ◑ prototyped (data model + build) — `Member.z_offset`; a non-zero offset builds the beam on *phantom* nodes at the offset elevation, each tied back to the drawn (slab) node via `RigidOffset(couple_slave_rotations=True)`, so the beam stiffness condenses onto the drawn nodes through the offset arm (full T-beam action, no support surgery). Same element tags as centroidal → results/design/selection map unchanged. Validated: monotonic stiffening (0.072→0.012 mm as offset 0→−0.5 m). 2 tests. **Remaining: GUI field** in the member dialog to set the offset. |
