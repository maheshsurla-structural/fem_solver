...
- 2026-09-13 — **Q2 done.** Completed the visual record's missing dimension — **density**.
  Alongside the per-surface light/dark pairs the L/A/S items already saved, there is now a
  uniform **2×2 montage per redesigned dialog** (`phase21_outputs/loads_ux/Q2_<surface>.png`):
  columns = light｜dark, rows = comfortable｜compact, for all ten dialogs (L2–L6, A1–A4),
  generated by reusing the Q1 construction harness (`_project()` + the surface map). Text
  renders as boxes in the headless `offscreen` grab (the app renders it normally on Windows) —
  the montages are the record for **layout, grouping, theming and density**, which do render.
  §10 below already carries the per-item before/after prose. **This closes the work stream:
...
- **Q2 density record** — every redesigned dialog now has a 2×2 montage
  ...
  image, so the theme and density behaviour of each surface is documented at a glance (e.g.
  `Q2_A2_nonlinear_case.png` shows the CSiBridge header + 2×2 card grid tightening from
  comfortable to compact in both themes).
