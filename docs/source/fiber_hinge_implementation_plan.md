...
- 2026‑09‑12 — **C3 completed: dynamic time‑history GUI.** The engine `run_time_history` (mass +
  Rayleigh damping + `ground_motion_force` + Newmark/Newton) now has a front end. Added a
  `step_callback` to `NonlinearTransientAnalysis` (additive; per‑step hook, return False cancels —
  mirrors the static one) and threaded it through `run_time_history` (`on_step`/`should_cancel`).
  `desktop/timehistory_dialog.py`: `load_accel_record` (free‑format / PEER‑style accelerogram parser,
  skips header lines), `TimeHistoryWorker` (QThread — streams progress, cooperative cancel),
  `TimeHistoryDialog` (monitor node + direction X/Y/Z, record import with dt / scale / m·s⁻² vs g,
  damping ζ, density; live progress bar + log; disp/velocity/acceleration response‑history plot with
  a quantity toggle). "Nonlinear time history…" wired into the Analysis menu (guarded on a fiber
  section). Verified: a fiber cantilever under a synthetic accelerogram produces the expected
...
| **C3** | **Dynamic time‑history** (true transient: mass + damping) for fiber elements | ☑ | Engine `desktop/nonlinear.run_time_history` (mass from `density`; Rayleigh damping to `zeta` at the first two modes; base motion `−M·ι·ü_g` via `ground_motion_force`; Newmark+Newton `NonlinearTransientAnalysis`). **GUI (2026‑09‑12):** `desktop/timehistory_dialog.py` — `load_accel_record` (free‑format/PEER‑style parser), `TimeHistoryWorker` (QThread; streams via a new `NonlinearTransientAnalysis.step_callback`; cooperative cancel), `TimeHistoryDialog` (monitor node/direction, record import + dt/scale/g‑units, ζ, density; live progress; disp/vel/accel response‑history plot). "Nonlinear time history…" in the Analysis menu. `test_desktop_timehistory.py` (3) + `test_desktop_timehistory_gui.py` (4). |
...
| C3 dynamic time‑history | ☑ | 2026‑09‑12 — engine run_time_history + GUI: TimeHistoryDialog (record import, ζ/direction/density, response plot), threaded via NonlinearTransientAnalysis.step_callback |
