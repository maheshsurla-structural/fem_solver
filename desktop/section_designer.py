...
class SectionDesignerWindow(QMainWindow):
    ...
    def _build_fibers_tab(self) -> QWidget:
        ...
        # left rail — discretisation inputs
        ctrl = QWidget()
        cv = QVBoxLayout(ctrl)
        cv.setContentsMargins(12, 12, 12, 12)
        cv.setSpacing(8)
        _fh = QLabel("INPUTS")
        _fh.setObjectName("ctrlHead")
        cv.addWidget(_fh)
        f = QFormLayout()
        ...
        cv.addWidget(self.fib_info)
        cv.addStretch(1)

        # right view — plot + fibre-vs-solid properties + fibre table
        view = QWidget()
        v = QVBoxLayout(view)
        v.setContentsMargins(0, 0, 0, 0)
        ...
        return self._workspace(ctrl, view)
