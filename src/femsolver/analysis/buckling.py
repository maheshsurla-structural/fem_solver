...
class LinearBucklingAnalysis:
    ...
    * ``Node.mode_disp[i, k]`` is the i-th DOF amplitude in the k-th
    ...
    def __init__(self, model, num_modes: int = 4, *, numberer: str = "default"):
        if num_modes < 1:
    ...
    def run(self) -> dict:
        ...
        # K_global on each element is the *initial* elastic stiffness;
        # K_tangent_global is the tangent at the current ``Node.disp``.
        K_sparse = assemble_stiffness(m)
        K_T_sparse = _assemble_tangent(m)
        K = np.asarray(K_sparse.toarray())
        K_T = np.asarray(K_T_sparse.toarray())
        K_g = K_T - K

        if np.max(np.abs(K_g)) < 1.0e-12 * max(1.0, np.max(np.abs(K))):
            raise RuntimeError(
        ...
        # Step 4: solve K_g v = nu K v on free DOFs.
        # scipy.linalg.eigh returns eigenvalues in ascending order.
        # Buckling load factor: lambda = -1 / nu, positive for the
        # compression-buckling branch (nu < 0).
        try:
            ...
            raise RuntimeError(
                ...
                "ill-conditioned. Check boundary conditions and load "
                "pattern."
            ) from exc

        # Compression-buckling branch: nu < 0 with magnitude well above
        # the numerical noise floor. The latter is set relative to the
        # largest |nu|, because ``eigh`` returns near-zero eigenvalues
        # at roughly ``eps * cond(K)`` even when no genuine buckling
        # exists; those correspond to load factors of order
        # ``1 / nu_noise`` which are unphysically large. ``1e-6 * scale``
        # is the practical cutoff in our problems (well-conditioned K
        # gives noise at ~1e-7 * scale or below).
        ...
        if not valid.any():
            raise RuntimeError(
                ...
                "floor). This typically means the load pattern does not "
                "create compression that softens K_g. Apply a compressive "
                "reference load (or flip the sign)."
            )
        nu_buck = nu_all[valid]
        vecs_buck = vecs_all[:, valid]
        lambdas = -1.0 / nu_buck
        # Sort ascending (smallest positive load factor first).
        order = np.argsort(lambdas)
        n_keep = min(self.num_modes, lambdas.size)
        lambdas = lambdas[order][:n_keep]
        vecs = vecs_buck[:, order][:, :n_keep]
